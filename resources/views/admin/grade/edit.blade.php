            ...
            <form action="/admin/grades/update/{{ $grade->id }}" method="post">
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $grade->nama) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type grade name" required>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="department_id" name="department_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            ...
                            @foreach($departements as $departement)
                                <option value="{{ $departement->id }}" {{ $grade->departement_id == $departement->id ? 'selected' : '' }}>{{ $departement->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
