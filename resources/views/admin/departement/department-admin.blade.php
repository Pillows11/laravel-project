<x-admin-layout>
    ...
    <div class="flex items-center justify-between mb-4">
        <!-- Tombol Add Grade -->
        <a href="/admin/departements/create"
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody class="rounded-b-lg">
            ...
            <tr class="bg-gray-50 hover:bg-blue-100 transition-colors duration-200">
                <td class="py-3 px-5 border-t">{{$departements->id}}</td>
                <td class="py-3 px-5 border-t">{{$departements->nama}}</td>
                <td class="py-3 px-5 border-t">{{$departements->descrip}}</td>
                <td class="px-6 py-4 text-center">
                    <div class="flex items-center justify-center space-x-2">
                        <button id="modalDetail" class="modalDetailBtn" data-id="{{ $departements->id }}"
                            data-name="{{ $departements->nama }}" data-departement="{{ $departements->descrip }}"
                        ...
                        <a href="/admin/departements/edit/{{ $departements->id }}">
                            <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                                xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                                viewBox="0 0 24 24">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-3 sm:space-x-4">
                    <a type="button" href="/admin/departements"
    ...
    document.addEventListener("DOMContentLoaded", function (event) {
        ...
        modalDetailBtns.forEach((btn) => {
            btn.addEventListener('click', function () {
                ...
                const departmentDesc = this.getAttribute('data-departement');

                modalName.textContent = departmentName;
                modalDepartment.textContent = departmentDesc;
                modal.classList.remove('hidden');
                modalAnchor.setAttribute('href', `/admin/departements/edit/${departmentId}`);
            });
        });
